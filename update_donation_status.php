...
if ($id === null || !in_array($status, DONATION_STATUSES, true) || (($status === 'Rejected' || $status === 'Cancelled') && $note === '')) { set_flash('error', 'Choose a valid status. A reason is required for rejected or cancelled donations.'); redirect_to('donation_detail.php?id=' . (int)$id); }
...
try {
    ...
    $verifiedAt = $status === 'Complete' ? date('Y-m-d H:i:s') : null;
    $update=$conn->prepare('UPDATE donations SET payment_status=?, status_note=?, verified_at=?, verified_by=? WHERE id=?'); $update->bind_param('ssssi',$status,$note,$verifiedAt,$admin,$id); $update->execute(); $update->close();
